(() => {
    ...
    style.textContent=`
#fcStartScreen,#fcResultScreen{position:fixed;inset:0;z-index:10000;display:flex;align-items:center;justify-content:center;padding:24px;background:#050505;color:#fff;font-family:Arial,sans-serif;overflow:auto}
...
.fcResultWin{color:#fff}.fcResultLose{color:#aaa}.fcResultDraw{color:#ddd}
#fcStats{display:none!important}
@media(max-width:600px){#fcStartScreen,#fcResultScreen{padding:14px}.fcPanel{padding:22px 18px;border-radius:16px}}
`;
    document.head.appendChild(style);

    const start=document.createElement('div');
    start.id='fcStartScreen';
    start.innerHTML=`<div class="fcPanel"><h1>FreeChess</h1><p>Bereit für die Partie?</p><label for="fcStartName">Username</label><input id="fcStartName" maxlength="24" autocomplete="nickname" placeholder="Dein Name"><label for="fcStartRating">Rating / Stärkezahl</label><input id="fcStartRating" type="number" min="0" max="4000" step="1" placeholder="1200"><button id="fcStartButton">Partie starten</button></div>`;
    document.body.appendChild(start);

    const result=document.createElement('div');
    result.id='fcResultScreen';
    result.style.display='none';
    result.innerHTML=`<div class="fcPanel"><h2 id="fcResultTitle">Partie beendet</h2><p id="fcResultText"></p><button id="fcResultExport">Schach-Log herunterladen</button><button id="fcResultNew" class="secondary">Neue Partie</button></div>`;
    document.body.appendChild(result);

    const nameInput=start.querySelector('#fcStartName');
    const ratingInput=start.querySelector('#fcStartRating');
    nameInput.value=localStorage.getItem('freechess_username')||'Player';
    ratingInput.value=Number(localStorage.getItem('freechess_rating'))||1200;
    ...
    function startGame(){
        ...
        const stats=document.getElementById('fcStats'); if(stats) stats.style.display='none';
        update(); drawBoard();
        if(typeof requestStockfishMove==='function' && !whiteTurn) requestStockfishMove();
    }
    ...
    function finishGame(kind){
        ...
        if(typeof stockfishAi!=='undefined' && stockfishAi && stockfishAi.busy && typeof stockfishAi.stop==='function'){try{stockfishAi.stop()}catch(_) {}}
        playStockFishMove=false;
        const title=document.getElementById('fcResultTitle'),text=document.getElementById('fcResultText');
        if(kind==='win'){title.textContent='Du hast gewonnen';title.className='fcResultWin';text.textContent='Stockfish ist besiegt.'}
        else if(kind==='lose'){title.textContent='Du hast verloren';title.className='fcResultLose';text.textContent='Stockfish hat die Partie gewonnen.'}
        else{title.textContent='Remis';title.className='fcResultDraw';text.textContent='Die Partie endet unentschieden.'}
        result.style.display='flex';
    }

    function checkGameEnd(){
        if(!gameStarted || gameFinished || !fcRules) return;
        if(!fcRules.isGameOver()) return;
        if(fcRules.isCheckmate()) finishGame(fcRules.turn()==='b'?'win':'lose');
    ...
    [nameInput,ratingInput].forEach(input=>input.addEventListener('keydown',e=>{if(e.key==='Enter') startGame()}));
    showStart();
