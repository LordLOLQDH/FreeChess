/* FREECHESS – MOBILE TAP CONTROLS – FIXED */
...
function mobileLegalDestinations(i){
    ...
    try{return fcRules.moves({square:mobileIndexToSquare(i),verbose:true}).map(m=>{return (8-Number(m.to[1]))*8+(m.to.charCodeAt(0)-97);});}catch{return[];}
}
function mobileRedrawSelection(){
    ...
    const r=Math.floor(mobileSelectedIndex/8),c=mobileSelectedIndex%8;
    ctx.strokeStyle='rgba(255,215,0,.95)'; ctx.lineWidth=4;
    ctx.strokeRect(c*sqreScale+2,r*sqreScale+2,sqreScale-4,sqreScale-4);
}
...
function mobileTryMove(from,to){
    ...
    let moved=false;
    try{moved=fcPlayerMove(mobileIndexToSquare(from),mobileIndexToSquare(to),'q');}catch{return false;}
...
function mobileHandleClick(e){
    // KEIN detail Check mehr!
    if(!whiteTurn||(typeof fcGameOver==='function'&&fcGameOver())){mobileClearSelection(); return;}
    if(isDown) return; // wenn gerade gedragt wird, nicht als Tap werten
    const idx=mobileIndexFromClientPoint(e.clientX,e.clientY);
    if(idx<0) return;
    const piece=board.boardArr[idx];
    if(mobileSelectedIndex===null){
        if(piece&&piece[0]==='w'){mobileSelectedIndex=idx; mobileRedrawSelection();}
        return;
    }
...
function setupMobileControls(){
    if(!cvs) {setTimeout(setupMobileControls,100); return;}
    cvs.addEventListener('click', mobileHandleClick);
}
setupMobileControls();
